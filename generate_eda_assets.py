...
def main():
    ...
    ax.set_xlabel("Signal oldidan necha kun")
    ax.set_ylabel("O'rtacha tranzaksiya soni")
    ax.set_title("Signal oldidan faollik: Escalated vs Dismissed")
    ax.legend()
    plt.tight_layout()
    ...
    try:
        ...
        if model_paths:
            ...
            ax.set_title("Top-15 eng muhim feature'lar (LightGBM)")
            plt.tight_layout()
            fig.savefig(args.out_dir / "feature_importance.png", dpi=120)
            plt.close(fig)
    except Exception as e:
        print(f"Feature importance grafigi o'tkazib yuborildi: {e}")
